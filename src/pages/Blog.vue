    ...
    <Layout>
        <section class="w-full h-full mb-16 pb-16">
            <ul id="revers" class="flex flex-col justify-around items-start">
                <li class="flex justify-around items-center h-full xs:h-12 overflow-hidden bg-gray-100 w-full"
                    v-for="{ node } in $page.post.edges" :key="node.id">
                    <div class="flex-1 flex flex-col justify-center items-center">
                        <div class="flex flex-col justify-center items-center">
                            <h3 class="font-bold text-ls mb-2 w-2/3 m-auto text-center">
                                <g-link :to="`blog/${node.slug}`"> {{ node.title }}</g-link>
                            </h3>
                            <vue-markdown class="text-gray-700 text-xs w-32 m-auto text-center">{{ excerpt(node) }}
                            </vue-markdown>
                        </div>
                        <!--<div class="">
